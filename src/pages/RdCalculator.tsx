...
import { calculateRd, RdCalculationParams, RdCalculationResult } from '../utils/calculatorUtils';
...
const RdCalculator: React.FC = () => {
  ...
  const [results, setResults] = useState<RdCalculationResult>({
    maturityValue: 0,
    totalInterest: 0,
    totalInvestment: 0,
    chartData: [],
  ...
  useEffect(() => {
    const params: RdCalculationParams = {
      monthlyInstallment,
      interestRate,
      timePeriod,
      considerInflation,
  ...
  const rdTableRows = results.chartData.filter(row => row.year > 0).map(row => {
    const formattedRow = {
      ...row,
  ...
  const tableSection = (
    <CalculatorTable columns={rdTableColumns} rows={rdTableRows} />
  );

  // Modern particulars section
  const particularsSection = (
    <ParticularsSection
      title="How RD is Calculated"
      items={[
        <><b>Monthly Deposit:</b> The fixed amount deposited every month.</>,
        <><b>Rate of Interest:</b> The annual interest rate.</>,
        <><b>Time Period:</b> Number of years the deposit is held.</>,
        <><b>Formula:</b> A = P × n + P × n(n+1)/2 × r/12/100</>,
      ]}
    />
  );
  const faqItems: FAQItem[] = [
    { q: 'What is a Recurring Deposit (RD)?', a: 'An RD is a savings scheme where you deposit a fixed amount every month for a fixed tenure.' },
    { q: 'How is RD maturity calculated?', a: 'A = P × n + P × n(n+1)/2 × r/12/100, where A is maturity value, P is monthly deposit, n is number of months, r is rate.' },
    { q: 'Is RD interest taxable?', a: 'Yes, RD interest is taxable as per your income tax slab.' },
    { q: 'Can I withdraw RD before maturity?', a: 'Yes, but you may incur a penalty and get a lower interest rate.' },
    { q: 'What is the minimum tenure for RD?', a: 'It varies by bank, but usually starts from 6 months.' },
  ];
  const faqSection = (
  ...
  return (
    <CalculatorTemplate
      title="RD Calculator"
      description="Calculate returns on your Recurring Deposits and plan your investments with our comprehensive calculator."
