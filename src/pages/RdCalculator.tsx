    ...
    <Box sx={{ mt: 3, mb: 2 }}>
      <Box sx={{ background: '#f4f7fa', borderRadius: 2, p: 2, mb: 2, display: 'flex', flexDirection: 'column', gap: 1, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        ...
        <Typography variant="body2" sx={{ color: colors.accent.primary, fontWeight: 500, fontFamily: 'JetBrains Mono, Fira Mono, monospace', fontSize: '1.02rem' }}>
          ...
          A = ₹{monthlyInstallment.toLocaleString('en-IN')} × (1 + {interestRate / 400})^{4 * timePeriod} = <b>{formatCurrency2(results.maturityValue)}</b>
        </Typography>
      </Box>
      <Box component="ul" sx={{ m: 0, pl: 2, color: colors.secondary, fontSize: { xs: '0.98rem', md: '1.03rem' }, lineHeight: 1.6, listStyle: 'none' }}>
        ...
        <Box component="li" sx={{ mb: 1.5, display: 'flex', alignItems: 'flex-start' }}>
          ...
          <span><b>n (Frequency):</b> Number of times interest is compounded per year (for RD, usually 4 for quarterly).</span>
        </Box>
        <Box component="li" sx={{ mb: 1.5, display: 'flex', alignItems: 'flex-start' }}>
        ...
        {[
          {
            ...
            a: 'A Recurring Deposit is a financial instrument where you deposit a fixed amount every month for a fixed tenure at a fixed interest rate.'
          },
          {
            q: 'How is RD interest calculated?',
            a: 'A = P × (1 + r/n)^(nt), where A is maturity value, P is monthly installment, r is rate, n is frequency, t is time.'
          },
          {
            q: 'What is compounding frequency in RD?',
            a: 'It is the number of times interest is added to the principal per year (for RD, usually quarterly = 4).' 
          },
          {
            q: 'How does inflation affect RD returns?',
          ...
          },
          {
            ...
            a: 'Yes, interest earned on RD is taxable as per your income tax slab.'
          }
        ].map((item, idx, arr) => (
